...
function updateWeather() {
  ...
  function getForecast(latitude, longitude) {
    ...
    fetch(api)
      ...
      .then(function(data) {
        forecast1.temperature.value = Math.floor(
          data.list[7].main.temp - KELVIN
        );
        forecast1.iconId = data.list[7].weather[0].icon;

        forecast2.temperature.value = Math.floor(
          data.list[15].main.temp - KELVIN
        );
        forecast2.iconId = data.list[15].weather[0].icon;

        forecast3.temperature.value = Math.floor(
          data.list[23].main.temp - KELVIN
        );
        forecast3.iconId = data.list[23].weather[0].icon;

        forecast4.temperature.value = Math.floor(
          data.list[31].main.temp - KELVIN
        );
        forecast4.iconId = data.list[31].weather[0].icon;

        forecast5.temperature.value = Math.floor(
          data.list[39].main.temp - KELVIN
        );
  ...
  function displayForecast() {
    ...
    foreIconElement1.innerHTML = `<img src="icons2/${forecast1.iconId}.png"/>`;

    foreTempElement2.innerHTML = `${forecast2.temperature.value}°<span>c</span>`;
    foreIconElement2.innerHTML = `<img src="icons2/${forecast2.iconId}.png"/>`;

    foreTempElement3.innerHTML = `${forecast3.temperature.value}°<span>c</span>`;
    foreIconElement3.innerHTML = `<img src="icons2/${forecast3.iconId}.png"/>`;

    foreTempElement4.innerHTML = `${forecast4.temperature.value}°<span>c</span>`;
    foreIconElement4.innerHTML = `<img src="icons2/${forecast4.iconId}.png"/>`;

    foreTempElement5.innerHTML = `${forecast5.temperature.value}°<span>c</span>`;
    foreIconElement5.innerHTML = `<img src="icons2/${forecast5.iconId}.png"/>`;
  }
